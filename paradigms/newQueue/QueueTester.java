 ...
 */
public class QueueTester<M extends newQueue.Queues.QueueModel> extends ArrayQueueTester<M> {
    public QueueTester(final Class<M> type, final newQueue.Queues.QueueChecker<M> tester, final Queues.Splitter<M> splitter) {
        super(type, tester, splitter);
    }

    public void test(final TestCounter counter) {
        test(counter, "LinkedQueue", newQueue.ReflectionTest.Mode.CLASS);
