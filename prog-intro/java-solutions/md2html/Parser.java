...
public class Parser {
    ...
    private void parseText() {
        String character = specialSymbols.getOrDefault(
            builder.charAt(position),
    ...
    private void parseBlock(final MarkdownElement block) {
        ...
        stack.clear();
        stack.add(block);
        highlightingCharacter1Count = 0;
        highlightingCharacter2Count = 0;
        for (int i = 0; i < builder.length(); i++) {
            if (i == 0 || builder.charAt(i - 1) != BACK_SLASH) {
