...
std::string parser::CommandParser::GetRiscvCommand(int addr, const std::string &str) { // TODO: Add support of symtable
    ...
    if (opcode == "0110011") {
        ...
        return GetFormatString(formatFor3Args, addr, code, instruction.c_str(), GetRegister(rd).c_str(),
        ...
        return GetFormatString(formatForLSJ, addr, code, instruction.c_str(), GetRegister(rd).c_str(),
                               std::to_string(imm).c_str(), GetRegister(rs1).c_str());
    } else if (opcode == "0100011") {
        int imm = (int) std::stoul(std::string(20, str[0]) + str.substr(0, 7) + str.substr(20, 5), nullptr, 2);
        std::string instruction = RiscVParser::ParseStoreOP(funct3);
        return GetFormatString(formatForLSJ, addr, code, instruction.c_str(), GetRegister(rs2).c_str(),
                               std::to_string(imm).c_str(),
                               GetRegister(rs1).c_str());
    } else if (opcode == "1100011") {
        int imm = (int) std::stoul(str.substr(0, 20) + std::string(12, '0'), nullptr, 2);
        std::string instruction = RiscVParser::ParseLogicalOP(funct3);
        return GetFormatString(formatFor3Args, addr, code, instruction.c_str(), GetRegister(rs1).c_str(),
                               GetRegister(rs2).c_str(), RiscVParser::GetLabel(imm + 4).c_str());
    } else if (opcode == "0110111" || opcode == "0010111") {
        int imm = (int) std::stoul(str.substr(0, 20) + std::string(12, '0'), nullptr, 2);
        std::string instruction = RiscVParser::ParseBitOP(opcode);
        return GetFormatString(formatFor2Args, addr, code, instruction.c_str(), GetRegister(rd).c_str(),
                               std::to_string(imm).c_str());
        ...
        int imm = (int) std::stoul(std::string(12, str[0]) + str.substr(12, 8) + str[11] + str.substr(1, 10) + "0",
                                   nullptr, 2);
        return GetFormatString(formatForLSJ, addr, code, instruction.c_str(), GetRegister(rd).c_str(),
                               GetRegister(rs1).c_str(), RiscVParser::GetLabel(imm).c_str());
    } else if (opcode == "1100111") {
        std::string instruction = "jalr";
        int imm = (int) std::stoul(std::string(20, str[0]) + str.substr(0, 12), nullptr, 2);
        return GetFormatString(formatForLSJ, addr, code, instruction.c_str(), GetRegister(rd).c_str(),
                               GetRegister(rs1).c_str(), std::to_string(imm).c_str());
...
std::string parser::CommandParser::GetFormatString(const std::string &format, Args... args) {
